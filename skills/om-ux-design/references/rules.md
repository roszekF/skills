...
## om-ux-design specifics

- Preserve the user's selected scope and settled decisions. Ask about missing
  product choices only when they affect the requested result.
- Use actual components and conventions when available. Portable assets and
  default tokens are proposals, not evidence of design-system conformance.
- Keep review identity, browser storage and exported operation history intact.
  Never overwrite a reviewed artifact through initialization.
- Run applicable source checks and browser walks before claiming verification.
  Human acceptance is separate and bound to a concrete version.
- Required report fields and their meanings live in `references/report-templates.md`.
  No tracker operation, remote publication or follow-on task is implied.
